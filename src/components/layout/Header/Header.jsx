...
export function Header() {
  ...
  const scheduleCloseMega = () => {
    if (closeTimerRef.current) clearTimeout(closeTimerRef.current)
    closeTimerRef.current = setTimeout(() => setMegaOpen(false), MEGA_CLOSE_DELAY)
  }
  ...
  return (
    <header
      className={clsx(styles.header, scrolled && styles.scrolled, megaOpen && styles.megaOpen)}
    >
      <div className={clsx('container', styles.inner)}>
        ...
        <nav className={styles.nav} aria-label={t('footer.quickLinks')}>
          ...
          <div
            ...
            onFocus={openMega}
            onBlur={(e) => {
              if (!e.currentTarget.contains(e.relatedTarget)) scheduleCloseMega()
            }}
          >
            <NavLink
              to="/services"
